...
class RMultKAN(nn.Module):
    ...
    def refine(self, new_grid):
        ...
        Returns:
        --------
            a refined model : RMultKAN
        '''

        model_new = RMultKAN(width=self.width, 
                     grid=new_grid, 
                     k=self.k, 
    ...
    def loadckpt(path='model'):
        ...
        model_load = RMultKAN(width=config['width'], 
                     grid=config['grid'], 
                     k=config['k'], 
    ...
    def rewind(self, model_id):
        ...
        return RMultKAN.loadckpt(path=self.ckpt_path+'/'+str(model_id))
    ...
    def update_grid(self, x):
        '''
        call update_grid_from_samples. This seems unnecessary but we retain it for the sake of classes that might inherit from RMultKAN
        '''
    ...
    def prune_node(self, threshold=1e-2, mode="auto", active_neurons_id=None, log_history=True):
        ...
        model2 = RMultKAN(copy.deepcopy(self.width), grid=self.grid, k=self.k, base_fun=self.base_fun_name, mult_arity=self.mult_arity, ckpt_path=self.ckpt_path, auto_save=True, first_init=False, state_id=self.state_id, round=self.round).to(self.device)
    ...
    def prune_input(self, threshold=1e-2, active_inputs=None, log_history=True):
        ...
        model2 = RMultKAN(copy.deepcopy(self.width), grid=self.grid, k=self.k, base_fun=self.base_fun, mult_arity=self.mult_arity, ckpt_path=self.ckpt_path, auto_save=True, first_init=False, state_id=self.state_id, round=self.round).to(self.device)
        model2.load_state_dict(self.state_dict())
    ...
    def expand_depth(self):
        ...
        layer = RKANLayer(dim_out, dim_out, num=self.grid, k=self.k)
        layer.mask *= 0.
        ...
        def _expand(layer_id, n_added_nodes, sum_bool=True, mult_arity=2, added_dim='out'):
            ...
            if sum_bool:
                ...
                if added_dim == 'out':
                    ...
                    self.act_fun[l] = RKANLayer(in_dim, out_dim + n_added_nodes, num=self.grid, k=self.k)
                ...
                if added_dim == 'in':
                    ...
                    self.symbolic_fun[l] = new
                    self.act_fun[l] = RKANLayer(in_dim + n_added_nodes, out_dim, num=self.grid, k=self.k)
                    self.act_fun[l].mask *= 0.
                ...
                if added_dim == 'out':
                    ...
                    self.symbolic_fun[l] = new
                    self.act_fun[l] = RKANLayer(in_dim, out_dim + n_added_subnodes, num=self.grid, k=self.k)
                    self.act_fun[l].mask *= 0.
